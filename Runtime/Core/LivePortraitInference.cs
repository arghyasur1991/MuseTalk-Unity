...
namespace MuseTalk.Core
{
    ...
    public class LivePortraitInference : IDisposable
    {
        ...
        public LivePortraitResult Generate(LivePortraitInput input)
        {
            ...
            try
            {
                ...
                for (int frameId = 0; frameId < Mathf.Min(maxFrames, input.DrivingFrames.Length); frameId++)
                {
                    ...
                    var pixelData = imgRgb.GetPixelData<byte>(0);
                    var imgRgbData = new byte[pixelData.Length];
                    pixelData.CopyTo(imgRgbData);
        ...
        private List<FaceDetectionResult> FaceAnalysis(byte[] img, int width, int height)
        {
            ...
            for (int y = 0; y < newHeight; y++)
            {
                for (int x = 0; x < newWidth; x++)
                {
                    ...
                    {
                        detImg[dstIdx] = resizedPixels[srcIdx];
                    }
                }
            }
            ...
            var inputTensor = PreprocessDetectionImage(detImg, newWidth, newHeight, inputSize);
        ...
        private DenseTensor<float> PreprocessDetectionImage(byte[] img, int width, int height, int inputSize)
        {
            var pixels = img;
            ...
            for (int dstY = 0; dstY < dsize; dstY++)
            {
                for (int dstX = 0; dstX < dsize; dstX++)
                {
                    ...
                    if (x0 >= 0 && (x0 + 1) < srcWidth && y0 >= 0 && (y0 + 1) < srcHeight)
                    {
                        ...
                        var c00 = srcPixels[unity_y0 * srcWidth * 3 + x0 * 3 + 0];
                        var c10 = srcPixels[unity_y0 * srcWidth * 3 + x0 * 3 + 1];
                        var c01 = srcPixels[unity_y1 * srcWidth * 3 + x0 * 3 + 0];
                        var c11 = srcPixels[unity_y1 * srcWidth * 3 + x0 * 3 + 1];

                        // Bilinear interpolation
                        float inv_fx = 1.0f - fx;
                        float inv_fy = 1.0f - fy;
                        
                        float r_float = inv_fx * inv_fy * c00 + fx * inv_fy * c10 + inv_fx * fy * c01 + fx * fy * c11;
                        float g_float = inv_fx * inv_fy * c00 + fx * inv_fy * c10 + inv_fx * fy * c01 + fx * fy * c11;
                        float b_float = inv_fx * inv_fy * c00 + fx * inv_fy * c10 + inv_fx * fy * c01 + fx * fy * c11;
